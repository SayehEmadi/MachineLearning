naiveBayeskhmtlb <- function(x, ...)
  UseMethod("naiveBayeskhmtlb")

naiveBayeskhmtlb.default <- function(x, y, laplace = 0, ...) {
  call <- match.call()
  ...
  class = "naiveBayeskhmtlb"
  )
}

naiveBayeskhmtlb.formula <- function(formula, data, laplace = 0, ...,
  ...
  if (is.data.frame(data)) {
    ...
    if (any(attr(Terms, "order") > 1))
      stop("naiveBayeskhmtlb cannot handle interaction terms")
    Y <- model.extract(m, "response")
    X <- m[,gsub("`", "", labels(Terms)), drop = FALSE]
    
    return(naiveBayeskhmtlb(X, Y, laplace = laplace, ...))
  ...
  } else stop("naiveBayeskhmtlb formula interface handles data frames or arrays only")
  
}


print.naiveBayeskhmtlb <- function(x, ...) {
...
predict.naiveBayeskhmtlb <- function(object,
                                     newdata,
                                     type = c("class", "raw"),
                                     threshold = 0.001,
                                     eps = 0,
...
model <- naiveBayeskhmtlb(Class ~ ., data = BreastCancer)
predict(model, BreastCancer[1:10,])
predict(model, BreastCancer[1:10,], type = "raw")
